...
Fecha: 2012/10/29
Descripcion: 	
	Se modifica valor default de la columna timeZoneRule de la tabla ccCamps de 0 a 1
	Se actualiza el campo timeZoneRule a 1 en la tabla ccCamps
	Se inserta setting para control remoto de agente a traves de Web Service
	Se modifica el SP ccsp_RIA_ABCAgents para borrar usuarios de las tablas ccMenu_ViewsUser y ccRIAWorkGroupUsers

Version requerida: 84
*/
set nocount on
declare @Version int
...
if @Version_Actual = @Version-1 -- Aqui poner numero de nueva version
 begin
	...
	---------------- inicio SCRIPT @Sql ----------------
		...
		set @Sql = 'DECLARE @ConstraintName nvarchar(200)

SELECT @ConstraintName = Name 
FROM SYS.DEFAULT_CONSTRAINTS 
WHERE PARENT_OBJECT_ID = OBJECT_ID(''ccCamps'') 
AND PARENT_COLUMN_ID = (SELECT column_id 
						FROM sys.columns 
						WHERE NAME = N''timeZoneRule'' 
						AND object_id = OBJECT_ID(N''ccCamps''))

IF @ConstraintName IS NOT NULL
    EXEC(''ALTER TABLE ccCamps DROP CONSTRAINT '' + @ConstraintName)

ALTER TABLE ccCamps 
ADD DEFAULT 1 FOR timeZoneRule'

	EXEC(@Sql)

		set @Sql = 'UPDATE ccCamps SET timeZoneRule = 1 WHERE timeZoneRule = 0'

	EXEC(@Sql)

		set @Sql = 'insert into ccsettings 
values (121,'''',''URL para los eventos del AgentWS'',1,''X'',''URL para disparar los eventos recibidos por el webservice del agente remoto'',''URL for the events triggered by the remote agent web service'',0)'
...
if @option=1--selected User
 begin
	select User_id,Login,Nombres,isnull(apellidoPaterno,''''),
	 isnull(ApellidoMaterno,''''),Sexo,canChangeStatus,isnull(IDArea,0)
