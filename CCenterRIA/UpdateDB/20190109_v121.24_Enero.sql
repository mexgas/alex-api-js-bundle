...
--Create-End
--Alter-Start
        set @process = 'CW-1933 Version 120.89.85.35 Se actualiza la base de datos temporal para el estado de las campañas con un tiempo menos o igual a 5 .'
...
if @Tipo in (1,2) begin
  ...
  update B set B.st= C.cam_procesando
  from #Tcamps A
  left join ccCampsNvosCB B  on A.cam_id=B.id
  left join ccCamps  C on C.cam_id=B.id
  ...
  --Se revisa que por lo menos una campaña se pueda actualizar para realizar el proceso en caso contrario se regresa el valor establecido
