...
BEGIN
	...
	set @process = 'CW-4990 Verificar si existe el ccso_riaregistrylists'
	set @sql = 'if exists (select * from sys.procedures where name = N''ccso_riaregistrylists'')
            begin
          DROP PROCEDURE ccso_riaregistrylists;
            end'
    EXEC(@sql)

	set @process = 'CW-4990 modificar el sp ccso_riaregistrylists para obtener la variable list_id'
	set @sql = '
	begin
