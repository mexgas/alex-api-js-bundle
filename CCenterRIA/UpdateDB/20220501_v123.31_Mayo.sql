...
set nocount off'
	...
	set @process = 'PREVIEW actualizacion del tiempo en previsualizacion'
	...
	set @process = 'PREVIEW eliminar si existe previamente setting 235 para preview'
    set @sql = 'delete ccsettings where setting_id=235'
	EXEC(@sql)

	set @process = 'PREVIEW se agrega setting para numero de descartes por tiempo'
    set @sql = 'insert ccsettings (setting_id,valor,descripcion,Status,Tipo,detalle,description,bLoadSettings,validate) 
		values (235, 3, ''Cantidad de veces que se exceda el tiempo maximo de previsualización antes de pasar a estado Inactivo'',
		1,''AGT'',''Cantidad de veces permitidas que se exceda el tiempo maximo de previsualización antes de pasar a estado Inactivo'',
		''Number of times that maximum preview time is exceeded before going to idle state'',1,''^\d{1,2}$'')'
	EXEC(@sql)
