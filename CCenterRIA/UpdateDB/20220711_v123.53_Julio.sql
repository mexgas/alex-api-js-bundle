...
END;'
   ...
   set @sql = 'ALTER PROCEDURE [dbo].[ccsp_WhatsAppInformation]
	...
	set @process = 'CW-6949 ST_2022_05_301 drop configuraIdiomaCatalogosEspañol'
    set @sql = 'IF EXISTS(SELECT 1 FROM sys.procedures WHERE Name = ''configuraIdiomaCatalogosEspañol'')
BEGIN
    DROP PROCEDURE dbo.configuraIdiomaCatalogosEspañol
END'
    EXEC(@sql)


	set @process = 'CW-6949 ST_2022_05_301 create configuraIdiomaCatalogosEspañol'
    set @sql='CREATE PROCEDURE [dbo].[configuraIdiomaCatalogosEspañol] AS
SET NOCOUNT ON
...
INSERT [dbo].[ccRIAChatMsg](descripcion, msg) values(''Default_Sp\Default13'', ''La sesión de chat ha concluido'')
	...
	set @process = 'CW-6949 ST_2022_05_301 drop configuraIdiomaCatalogosEnglish'
    set @sql = 'IF EXISTS(SELECT 1 FROM sys.procedures WHERE Name = ''configuraIdiomaCatalogosEnglish'')
BEGIN
    DROP PROCEDURE dbo.configuraIdiomaCatalogosEnglish
END'
    EXEC(@sql)


	set @process = 'CW-6949 ST_2022_05_301 create configuraIdiomaCatalogosEnglish'
