...
BEGIN
	...
	BEGIN TRY
	 ...
	 ------------------------------------------------------------ DEV2-17_K004022, DEV2-3-K004023 ---------------------------------------------------------------------
        ...
        set @sql = '
			if not exists (select * from sys.tables where name = N''ccCampsInfo'')
			begin

				CREATE TABLE [dbo].[ccCampsInfo](
					[id] [int] IDENTITY(1,1) NOT NULL,
					[cam_id] [smallint] NOT NULL,
					[contact_reg] [decimal](5, 2) NOT NULL default 0,
					[dial_retries] [decimal](5, 2) NOT NULL default 0,
...
------------------------------------------------------------  Inicia Ciro ---------------------------------------------------------------------
        ...
        set @sql = 'CREATE PROCEDURE  [dbo].[ccsp_UpdateOutWhatsappConfig] 
			...
			BEGIN
				UPDATE ccCamps SET cam_tnotas = @tNotas, cam_ShowCalifWnd = @ShowCalifWnd, exitAssisted = @ExitAssisted, chat = 5 where cam_id = @outbound_id;
			END;
			SELECT @outbound_id;
	...
	values(237,''C:\Centerware\ReplData2'',''Ruta donde se guardaran las replicas'',1,''GRL'',''Ruta Replicas'',''Path replication'',0,''.*'')
        ...
        set @sql = 'CREATE PROCEDURE [dbo].[ccsp_RIAConfCamp]
			...
			isnull(exitAssisted, 0) exitAssisted, isnull(previewDiscard, 0) PreviewDiscard, isnull(rotativeAlgo, 0 ) rotativeAlgo, isnull(timesPreview,0) TimesPreview, cam_tPreview, isnull(chat, 0) Chat, isnull(contact.conexionInfo,'''') conexionInfo, isnull(contact.closeConversationTime,0) closeConversationTime, isnull(contact.answerTimeoutClient,0) answerTimeoutClient, isnull(contact.allowFileAttachments,0) allowFileAttachments
		...
		set @process = 'CREATE procedure [ccsp_GalateaGetOutboundConfiguration]'
        set @sql = 'CREATE PROCEDURE [dbo].[ccsp_GalateaGetOutboundConfiguration]
				...
				BEGIN
					...
					prefijo varchar(40),enbleprefix bit,exitAssisted bit,previewDiscard bit , rotativeAlgo tinyint , timesPreview smallint, cam_tPreview smallint, chat int, conexionInfo varchar(15), closeConversationTime smallint, answerTimeoutClient int, allowFileAttachments bit)
