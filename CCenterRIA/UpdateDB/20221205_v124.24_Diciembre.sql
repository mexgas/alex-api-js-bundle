...
BEGIN
    ...
    set @sql = '
	    ...
	    begin
	        alter table ccCamps add timesDiscard tinyint not null default 6
	    end
	'
    EXEC(@sql)
    ...
    set @sql = '
		...
		exec @result_maxtimesdiscard = ccsp_CheckTimesDiscard @action=1,@camId=@camId, @calloutId=@callout_id
    ...
    set @sql = '		
		CREATE PROCEDURE [dbo].[ccsp_RIAConfCamp]
			...
			isnull(exitAssisted, 0) exitAssisted, isnull(previewDiscard, 0) PreviewDiscard, isnull(rotativeAlgo, 0 ) rotativeAlgo, isnull(timesPreview,0) TimesPreview, cam_tPreview, isnull(chat, 0) Chat, isnull(contact.conexionInfo,'''') conexionInfo, isnull(contact.closeConversationTime,0) closeConversationTime, isnull(contact.answerTimeoutClient,0) answerTimeoutClient, isnull(contact.allowFileAttachments,0) allowFileAttachments, isnull(timesDiscard,0) TimesDiscard
    ...
    set @sql = '		
		CREATE PROCEDURE [dbo].[ccsp_GalateaGetOutboundConfiguration]
				...
				BEGIN
					...
					prefijo varchar(40),enbleprefix bit,exitAssisted bit,previewDiscard bit , rotativeAlgo tinyint , timesPreview smallint, cam_tPreview smallint, chat int, conexionInfo varchar(15), closeConversationTime smallint, answerTimeoutClient int, allowFileAttachments bit, timesDiscard tinyint)
						...
						conexionInfo ConexionInfo, closeConversationTime CloseConversationTime, answerTimeoutClient MUTimeOutClient, allowFileAttachments AllowFileAttachments, timesDiscard TimesDiscard
