...
BEGIN
	...
	BEGIN TRY
		...
		SET @sql = 'ALTER procedure [dbo].[ccsp_RIAAbandon_Config]
					...
					set nocount off';
		EXEC(@sql);
		--------------------------------------- SE AGREGA SP ccsp_RIAUpdateCamConfig PARA LA TAREA DE ESQUI -----------------------
		...
		SET @process = 'Se agrega el parámetro CampType y se agrega case para diferenciar entre campañas de salida, de VP y de IA, lineas(1122 y 1118)'
		SET @sql = 'CREATE PROCEDURE [dbo].[ccsp_RIAUpdateCamConfig]
	                ...
	                UPDATE ccCamps SET
	                 ...
	                 CampType = (CASE WHEN @progDial = 3 THEN @progDiaL WHEN @CampType != NULL THEN @CampType ELSE 1 END)
