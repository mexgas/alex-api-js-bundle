...
BEGIN
	...
	BEGIN TRY
		------------------------------------------------- BEGIN Marco Garcia K034000-Carga BD-Registros no cargados o actualizados ----------------------------------------------------------------------
			SET @process = 'K034000-Carga BD-Registros no cargados o actualizados delete table fileHeadersPhoneLoad'
	SET @sql = 'IF EXISTS (SELECT * FROM sysobjects WHERE name=''fileHeadersPhoneLoad'' and xtype=''U'')
		BEGIN
			DROP TABLE [dbo].fileHeadersPhoneLoad;
		END'
	EXEC(@sql)

	SET @process = 'K034000-Carga BD-Registros no cargados o actualizados create table fileHeadersPhoneLoad';
	SET  @sql = 'CREATE TABLE fileHeadersPhoneLoad(
		load_id INT NOT NULL PRIMARY KEY,
		header_phone VARCHAR(MAX) NULL,
		header_phone2 VARCHAR(MAX) NULL,
		header_phone3 VARCHAR(MAX) NULL,
		header_phone4 VARCHAR(MAX) NULL,
		header_phone5 VARCHAR(MAX) NULL)'
	EXEC(@sql)

	SET @process = 'K034000-Carga BD-Registros no cargados o actualizados delete store procedure ccsp_RIALogPhones'
	...
	SET @process = 'K034000-Carga BD-Registros no cargados o actualizados create store procedure ccsp_RIALogPhones'
				...
				BEGIN
					SET @sql = ''SELECT * FROM (select  
						...
						CASE  WHEN crlp.tipoMov = 2 THEN ''''N/A'''' 
							  WHEN (crlp.tipoMov = 1 OR crlp.tipoMov = 4) AND ''+ @language + '' = 0  THEN ''''''+@typeDescriptionPhoneBlackList+''''''
	...
	EXEC (@sql)
			...
			------------------------------------------------- END Marco Garcia K034000-Carga BD-Registros no cargados o actualizados ----------------------------------------------------------------------
