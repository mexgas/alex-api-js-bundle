...
BEGIN
	...
	BEGIN TRY
	---------------------------------------Begin Rod Salazar  ---------------------------------------------------------
	SET @process = 'KR076000 Crear columna selectRotativeANI en tabla ccCamps'
	SET @sql = 'IF NOT EXISTS (SELECT * FROM sys.columns WHERE name = N''selectRotativeANI'' AND Object_ID = Object_ID(N''ccCamps''))
				BEGIN
					ALTER TABLE ccCamps ADD selectRotativeANI int NULL
				END
	'
	EXEC(@sql)
	...
	SET @process = 'KR076000 Validación y eliminación de sp ccsp_RIACampsManualCall'
	...
	SET @process = 'KR076000 Creación de sp ccsp_RIACampsManualCall'
