...
BEGIN
	...
	BEGIN TRY
	SET @process = 'delete sp [ccsp_RIALoadCamps]'
	SET @sql = '
	if exists (select * from sys.procedures where name = N''ccsp_RIALoadCamps'')
	begin
	...
	SET @process = 'create sp [ccsp_RIALoadCamps]'
	SET @sql = '
	CREATE PROCEDURE [dbo].[ccsp_RIALoadCamps] @option SMALLINT, @AreaId SMALLINT = NULL, @Sup SMALLINT = NULL, @WGID SMALLINT = NULL
					...
					BEGIN
						SELECT a1.cam_id, cam_descripcion, frame, cam_procesando, isnull(IDArea, ''''), isnull(DNCscrub, '''')
						FROM ccCamps a1
					...
					BEGIN
						SELECT DISTINCT a1.cam_id, cam_descripcion, frame, cam_procesando, isnull(IDArea, '''') IDArea, dbo.fn_CampEspWG(a1.cam_id, 3) relationsWG
						FROM ccCamps a1
						JOIN ccRIACampsGraph a2 ON a1.cam_id = a2.cam_id
						JOIN ccRIAGraphics a3 ON a2.graphic_id = a3.graphic_id
						WHERE a3.type_id = 1 AND isnull(IDArea, '''') = isnull(@AreaId, '''')
					...
					BEGIN
						SELECT DISTINCT a1.cam_id, a1.cam_descripcion, a3.frame, a1.cam_procesando, isnull(a1.IDArea, '''') IDArea
						FROM ccCamps a1
						JOIN ccRIACampsGraph a2 ON a1.cam_id = a2.cam_id
						JOIN ccRIAGraphics a3 ON a2.graphic_id = a3.graphic_id
					...
					BEGIN
						...
						FROM (
							SELECT A.LOGIN, A.User_id, Prioridad, Skill, C.cam_id, C.cam_descripcion, isnull(C.IDArea, '''') IDArea, CA.rel_id
							FROM ccCamps C
							JOIN ccCampsAgente CA ON C.cam_id = CA.cam_id
							JOIN ccRIACampsGraph a2 ON C.cam_id = a2.cam_id
							JOIN ccRIAGraphics a3 ON a2.graphic_id = a3.graphic_id
							JOIN ccUsers A ON A.User_id = CA.User_id AND A.TipoUser_id = 1 AND A.STATUS = 1 AND (@loginDays = '''' OR DATEDIFF(dd, LastLoginAttempt, getdate()) <= @loginDays)
							WHERE C.cam_id IN (
									SELECT cam_id
									FROM ccsupervisorcam
									WHERE user_id = CASE isnull(@Sup, '''') WHEN '''' THEN user_id ELSE @Sup END AND tipo = 1
									)
							) Relations
					...
					BEGIN
						...
						SELECT DISTINCT Camps.cam_id, Camps.cam_descripcion, a3.frame, Camps.cam_procesando, isnull(Camps.IDArea, '''') IDArea, IsNull(CN.New, '''') AS New, IsNull(CN.CB, '''') AS CB, IsNull(CN.Pro, '''') AS Pro, IsNull(CN.pen, '''') AS Pen, 
						cast(Camps.cam_procesando AS INT) AS St, Camps.cam_TipoJobs AS Job, isnull(CN.Fin, '''') Fin, isnull(CP.prioridad, ''12345NNN'') prioridad, cast(camps.dialorder AS TINYINT) dialorder, 
					...
					BEGIN
						SELECT DISTINCT a1.cam_id, a1.cam_descripcion, a3.frame, a1.cam_procesando, isnull(IDArea, '''') IDArea, isnull(DNCscrub, '''') DNCScrub
						FROM ccCamps a1
						JOIN ccRIACampsGraph a2 ON a1.cam_id = a2.cam_id
						JOIN ccRIAGraphics a3 ON a2.graphic_id = a3.graphic_id
						WHERE a3.type_id = 1 AND isnull(a1.cam_id, '''') = isnull(@AreaId, '''')
						ORDER BY 5, 2
					...
					BEGIN
						(SELECT DISTINCT a1.cam_id as CamID, cam_descripcion as CamDescription, frame as Frame, isnull(IDArea, '''') IDArea, dbo.fn_CampEspWG(a1.cam_id, 3) as RelationsWG,1 CamType, 
						ISNULL((select  count(IdCampEsp) from ccRIACampEspWG where tipo = 1 and IdCampEsp = a1.cam_id and IDWG = @WGID group by IdCampEsp),'''') IsAssignedToCurrentWG,
						CAST(CASE WHEN a1.progDial = 3 THEN 6 WHEN a1.CampType = 5 THEN 5 WHEN a1.CampType=7 THEN 7 ELSE '''' END as [tinyint]) [MediaType]
						FROM ccCamps a1
						JOIN ccRIACampsGraph a2 ON a1.cam_id = a2.cam_id
						JOIN ccRIAGraphics a3 ON a2.graphic_id = a3.graphic_id
						WHERE a3.type_id = 1 AND isnull(IDArea, '''') = isnull(@AreaId, '''')
						UNION
						SELECT DISTINCT b1.inbound_id, descripcion, frame, isnull(IDArea, '''') IDArea, dbo.fn_CampEspWG(b1.inbound_id, 2) relationsWG, '''' CampType, 
						ISNULL((select  count(IdCampEsp) from ccRIACampEspWG where tipo = '''' and IdCampEsp = b1.inbound_id and IDWG = @WGID group by IdCampEsp),'''') isAssignedToCurrentWG,
						b1.chat [MediaType]
						FROM ccinbound b1
						JOIN ccRIAinboundGraph b2 ON b1.inbound_id = b2.inbound_id
						INNER JOIN ccRIAGraphics b3 ON b2.graphic_id = b3.graphic_id
						LEFT JOIN (
							SELECT inbound_id, CASE WHEN (sum(skill) / count(user_id)) = max(skill) THEN '''' ELSE 1 END skillDif
							FROM ccSkills
							GROUP BY inbound_id
							) S ON S.Inbound_id = b1.inbound_id
						WHERE b3.type_id = 1 AND isnull(IDArea, '''') = isnull(@AreaId, '''')
