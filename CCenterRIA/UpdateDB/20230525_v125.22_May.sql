...
BEGIN
	...
	set @sql = 'IF EXISTS(SELECT 1 FROM sys.procedures WHERE Name = ''ccspGalatea_Finder'')
            BEGIN
                DROP PROCEDURE [dbo].[ccspGalatea_Finder]
            END'
    EXEC(@sql)

	set @process = 'KR082000 create SP ccspGalatea_Finder'
	set @sql = '
