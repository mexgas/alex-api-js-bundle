...
begin
	...
	SET @process = 'CW-8138 Se agrega nueva columna Unauthorized a la tabla ccSmsConversationsResult'
	SET @sql = 'IF NOT EXISTS(SELECT * FROM sys.columns WHERE name = N''Unauthorized'' AND Object_ID = Object_ID(N''ccSmsConversationsResult''))
				BEGIN
					ALTER TABLE ccSmsConversationsResult ADD Unauthorized INT DEFAULT 0;
				END'
	EXEC(@sql)

	SET @process = 'CW-8138  Se hace drop a la tabla ccspOutboundSmsMessage'
	...
	SET @process = 'CW-8138 Se agrega nueva columna Unauthorized en inserts a tabla y se agrega action 9 para traer status de los mensajes'
...
if @action=1 begin
	...
	if not exists(select * from ccSmsConversationsResult where camId=@camId) begin
		insert into ccSmsConversationsResult values(@camId,@SentMsg,0,0,0,0,@InsufficientBalance, @Unauthorized)
	end
	else begin
...
delete from smsWorkingTable where smsout_id in(''+@smsoutIds+'')
	...
	(
		value
		for description in (SentMsg,Delivered,NotDelivered,RecipientRejected,CarrierRejected,InsufficientBalance, Unauthorized)
	) unpiv
	where camId= @camId
	...
	SELECT A.smsout_id AS SmsOutId
		 , A.sms_status AS SmsStatus
	FROM smsOutSource A
	INNER JOIN smsWorkingTable B ON A.smsout_id = B.smsout_id
	WHERE B.cam_id = @camId
	ORDER BY A.smsout_id
end'
EXEC(@sql)
