...
BEGIN
	...
	--------------------------------------------------------------------START HL----------------------------------------------------------------------------------------
	SET @process = 'DEV2-216-Drop SP ccsp_GalateaAdminSetPermissions'
	SET @sql = 'IF EXISTS(SELECT 1 FROM sys.procedures WHERE Name = ''ccsp_GalateaAdminSetPermissions'')
			BEGIN
				DROP PROCEDURE [dbo].[ccsp_GalateaAdminSetPermissions]
			END'
	EXEC(@sql)

	SET @process = 'DEV2-216-Create SP ccsp_GalateaAdminSetPermissions'
	SET @sql = 'CREATE PROCEDURE [dbo].[ccsp_GalateaAdminSetPermissions]
		...
		BEGIN
			...
			SET DialMask =
				...
				THEN 
					CASE
					WHEN @permissionValue & 1 = 1
					THEN 1
					WHEN @permissionValue & 1 = 0
					THEN 0
					END
	...
	SET @process = 'KR093000-Drop SP ccsp_EngineLogTransfers'
	SET @sql = 'IF EXISTS(SELECT 1 FROM sys.procedures WHERE Name = ''ccsp_EngineLogTransfers'')
			BEGIN
				DROP PROCEDURE [dbo].[ccsp_EngineLogTransfers]
			END'
	EXEC(@sql)

	SET @process = 'KR093000-Create SP ccsp_EngineLogTransfers'
	SET @sql = 'CREATE PROCEDURE [dbo].[ccsp_EngineLogTransfers]
		...
		if @action = 1 begin
			if @modo = 4 begin
				insert into ccLogTransfers(cal_id,tipo,modo,destino,tAntesXfer,tDespuesXfer,fechaFin,pbxId,channel, tipoLlamada_id, callerAni) 
				values ( @cal_id, @tipo, @modo, @destino, @tantes, @tdespues, @xferDate, @pbxId, @channel, dbo.fnGetTipoLlamada(@destino), @callerAni)
				if @tdespues > 0 begin
				...
				begin
					insert into ccLogTransfers(cal_id,tipo,modo,destino,tAntesXfer,tDespuesXfer,fechaFin,pbxId,channel, tipoLlamada_id, callerAni) 
					values ( @cal_id, @tipo, @modo, @destino, 0, @tantes, @xferDate, @pbxId, @channel, dbo.fnGetTipoLlamada(@destino), @callerAni)
				end
	...
	---------------------------------------END Enrique Ruiz-----------------------------------------------------------
	---------------------------------------BEGIN Frida Orta-----------------------------------------------------------
	SET @process = 'Delete sp ccsp_AgentGetCalificaciones'
	...
	SET @process = 'Create sp ccsp_AgentGetCalificaciones'
	SET @sql = '
