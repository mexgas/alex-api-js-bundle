...
--------------------------------------------------------------------- END Gaby --------------------------------------------------------------------------
	...
	SET @process = 'Recepcion de mensajes de whatsapp'
	SET @sql = '
	    IF EXISTS (SELECT * FROM sys.procedures where name = N''ccsp_ConversationWASave'')
	    BEGIN
	...
	SET @process = 'Recepcion de mensajes de whatsapp'	
	SET @sql='
		CREATE PROCEDURE [dbo].[ccsp_ConversationWASaveOut] @action             INT
		, @conversationId     INT         = 0
		, @camId          SMALLINT    = NULL
