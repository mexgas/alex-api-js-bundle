...
SET @process = 'K070061, K070062, K070063 - add records to ccGalateaOperations, ccGalateaIdentifiers y relationTableColumnIdentifiers'
	SET @sql = '
	
-- Operacion Eliminar Campaña de Entrada IA
 IF NOT EXISTS (
    SELECT 1
    FROM ccGalateaOperations
    WHERE OperationId = 139
)
BEGIN
    INSERT INTO ccGalateaOperations (
        OperationId,
        OpTagEs,
        OpTagEn,
        OpTagPt
    ...
    VALUES (
        139,
        ''Eliminar campaña (llamada de entrada IA)'',
        ''Delete campaign (AI inbound call)'',
        ''Excluir campanha (chamada de entrada IA)''
    )
END

 -- Operacion Editar Campaña de Entrada IA
 IF NOT EXISTS (
    SELECT 1
    FROM ccGalateaOperations
    WHERE OperationId = 138
)
BEGIN
    INSERT INTO ccGalateaOperations(
			OperationId,
    ...
    VALUES (
		   138,
           ''Editar campaña (llamada de entrada IA)'',
           ''Edit campaign (AI inbound call)'',
           ''Editar campanha (chamada de entrada IA)''
	)
END


-- Identificador para TransferOnFallback_Mode
IF NOT EXISTS (
    SELECT * FROM ccGalateaIdentifiers 
    WHERE Description = ''IN_CALL_IA_TRANSFER_ON_FALLBACK_MODE''
)
BEGIN
    INSERT INTO ccGalateaIdentifiers (Description, TagEs, TagEn, TagPt)
    VALUES (
        ''IN_CALL_IA_TRANSFER_ON_FALLBACK_MODE'',
...
IF NOT EXISTS (
    ...
    WHERE Identifiers = ''IN_CALL_IA_TRANSFER_ON_FALLBACK_MODE'' AND tableName = ''ccInboundExtend''
)
BEGIN
    INSERT INTO relationTableColumnIdentifiers (Identifiers, tableName, colunName)
    VALUES (''IN_CALL_IA_TRANSFER_ON_FALLBACK_MODE'', ''ccInboundExtend'', ''TransferOnFallback_Mode'')
END

-- Identificador para TransferOnFallback_TimeoutSec
IF NOT EXISTS (
    SELECT * FROM ccGalateaIdentifiers 
    WHERE Description = ''IN_CALL_IA_TRANSFER_ON_FALLBACK_TIMEOUTSEC''
)
BEGIN
    INSERT INTO ccGalateaIdentifiers (Description, TagEs, TagEn, TagPt)
    VALUES (
        ''IN_CALL_IA_TRANSFER_ON_FALLBACK_TIMEOUTSEC'',
        ''Tiempo máximo de transferencia a agentes humanos'',
        ''Maximum live agent transfer time'',
        ''Tempo máximo de transferência para agentes humanos''
    )
END

IF NOT EXISTS (
    SELECT * FROM relationTableColumnIdentifiers 
    WHERE Identifiers = ''IN_CALL_IA_TRANSFER_ON_FALLBACK_TIMEOUTSEC'' AND tableName = ''ccInboundExtend''
)
BEGIN
    INSERT INTO relationTableColumnIdentifiers (Identifiers, tableName, colunName)
    VALUES (''IN_CALL_IA_TRANSFER_ON_FALLBACK_TIMEOUTSEC'', ''ccInboundExtend'', ''TransferOnFallback_TimeoutSec'')
END

-- Identificador para TransferOnSuccess_Mode
IF NOT EXISTS (
    SELECT * FROM ccGalateaIdentifiers 
    WHERE Description = ''IN_CALL_IA_TRANSFER_ON_SUCCESS_MODE''
)
BEGIN
    INSERT INTO ccGalateaIdentifiers (Description, TagEs, TagEn, TagPt)
    VALUES (
        ''IN_CALL_IA_TRANSFER_ON_SUCCESS_MODE'',
        ''Transferencia por gestión exitosa'',
        ''Successful interaction'',
        ''Transferência de interação bem-sucedida''
    )
END

IF NOT EXISTS (
    SELECT * FROM relationTableColumnIdentifiers 
    WHERE Identifiers = ''IN_CALL_IA_TRANSFER_ON_SUCCESS_MODE'' AND tableName = ''ccInboundExtend''
)
BEGIN
    INSERT INTO relationTableColumnIdentifiers (Identifiers, tableName, colunName)
    VALUES (''IN_CALL_IA_TRANSFER_ON_SUCCESS_MODE'', ''ccInboundExtend'', ''TransferOnSuccess_Mode'')
END

-- Identificador para TransferOnSuccess_TimeoutSec
IF NOT EXISTS (
    SELECT * FROM ccGalateaIdentifiers 
    WHERE Description = ''IN_CALL_IA_TRANSFER_ON_SUCCESS_TIMEOUTSEC''
)
BEGIN
    INSERT INTO ccGalateaIdentifiers (Description, TagEs, TagEn, TagPt)
    VALUES (
        ''IN_CALL_IA_TRANSFER_ON_SUCCESS_TIMEOUTSEC'',
        ''Tiempo máximo de transferencia por gestión exitosa'',
        ''Maximum successful interaction transfer time'',
        ''Tempo máximo de transferência de interação bem-sucedida''
    )
END

IF NOT EXISTS (
    SELECT * FROM relationTableColumnIdentifiers 
    WHERE Identifiers = ''IN_CALL_IA_TRANSFER_ON_SUCCESS_TIMEOUTSEC'' AND tableName = ''ccInboundExtend''
)
BEGIN
    INSERT INTO relationTableColumnIdentifiers (Identifiers, tableName, colunName)
    VALUES (''IN_CALL_IA_TRANSFER_ON_SUCCESS_TIMEOUTSEC'', ''ccInboundExtend'', ''TransferOnSuccess_TimeoutSec'')
END

-- Modo de Transferencia Ciega
IF NOT EXISTS (
    SELECT * FROM ccGalateaIdentifiers 
    WHERE Description = ''IN_CALL_IA_TRANSFER_BLIND_MODE''
)
BEGIN
    INSERT INTO ccGalateaIdentifiers (Description, TagEs, TagEn, TagPt)
    VALUES (
        ''IN_CALL_IA_TRANSFER_BLIND_MODE'',
        ''modalidad ciega'',
        ''blind mode'',
        ''modalidade cega''
    )
...
-- Modo de Transferencia Asistida
IF NOT EXISTS (
    SELECT * FROM ccGalateaIdentifiers 
    WHERE Description = ''IN_CALL_IA_TRANSFER_ASSISTED_MODE''
)
BEGIN
    INSERT INTO ccGalateaIdentifiers (Description, TagEs, TagEn, TagPt)
    VALUES (
        ''IN_CALL_IA_TRANSFER_ASSISTED_MODE'',
        ''modalidad asistida'',
        ''assisted mode'',
...
--Cambios para K070063 - Eliminar campaña de entrada de IA - JM 
	...
	SET @process = 'K070063 Drop SP ccsp_GalateaDeleteCampaignAndACD'
    ...
    EXEC(@sql);
	 ...
	 SET @process = 'K070063 CREATE SP ccsp_GalateaDeleteCampaignAndACD'
...
--Cambios para K070063 - Editar campaña de entrada de IA - JM 
    ...
    BEGIN
        DROP PROCEDURE dbo.ccsp_GalateaUpdateVoiceConfiguration;
    END'
    EXEC(@sql);

	 SET @process = 'K070061 - K070062 CREATE SP ccsp_GalateaUpdateVoiceConfiguration'
...
BEGIN
	...
	SELECT @chatType = chat FROM ccInbound WHERE Inbound_id = @inboundId;

	DECLARE @operation SMALLINT;

	IF @chatType = 11
	BEGIN
			SET @operation = 138 -- Editar campaña IA
	END
	ELSE
	BEGIN
			SET @operation = 52 -- Editar campaña normal
	END
...
END	
	...
	SET @process = 'K070061 - K070062 Drop SP ccsp_GalateaGetInboundConfiguration'

    SET @sql = 'if exists (select * from sys.procedures where name = N''ccsp_GalateaGetInboundConfiguration'')
    BEGIN
        DROP PROCEDURE dbo.ccsp_GalateaGetInboundConfiguration;
    END'
    EXEC(@sql);

	 SET @process = 'K070061 - K070062 CREATE SP ccsp_GalateaGetInboundConfiguration'
...
DECLARE @maxLoad INT = 400;
DECLARE @minPerAgent INT = 20;

SET @topCount = 
    CASE 
        WHEN @nAgentsLogin <= 0 THEN 20
        ELSE 
            CASE 
                WHEN (@nAgentsLogin * @minPerAgent) > @maxLoad THEN @maxLoad
                ELSE @nAgentsLogin * @minPerAgent
            END
    END;
...
'
    ...
    SET @process = 'ALTER PROCEDURE [dbo].[ccsp_OUTResetJobs] se agrega (ROWLOCK, UPDLOCK) '
...
BEGIN
  ...
  IF @today IS NULL
    SET @today = CONVERT(DATETIME, CONVERT(VARCHAR(11), GETDATE(), 121), 121);
  ELSE
  ...
  -- Fase 1: Actualizar registros que se acaban de marcar
  UPDATE wt WITH (ROWLOCK, UPDLOCK)
  SET wt.cal_status = 1
  FROM ccoWorkingTable wt
  INNER JOIN #TempccoLogDials ld ON wt.callout_id = ld.callout_id
  WHERE wt.cal_status = 2;

  -- Fase 2: Actualizar registros nuevos (sin necesidad de transacción global)
  IF @camid = 0
  BEGIN
  ...
  BEGIN
    UPDATE ccoWorkingTable WITH (ROWLOCK, UPDLOCK)
    SET cal_status = 0
    WHERE cal_status = 2 AND cam_id = @camid;
  END
...
END'
    ...
    SET @process = 'ALTER TABLE dbo.xxClienteCarga ADD CONSTRAINT PK_clienteCarga PRIMARY KEY CLUSTERED (cuenta, cam_id)'
...
'
    ...
    SET @process = 'Alter SP xx_ChecaHorario'
    SET @sql = 'ALTER procedure [dbo].[xx_ChecaHorario]
as
select 1 as ok  -- valido (fuera de horario de operaciones)'
    EXEC(@sql)

    SET @process = 'ALTER PROCEDURE [dbo].[xx_OUTInsertNewJOBS_WT_Camp] Se mejora para no tener bloques de base datos con la DLL'
...
BEGIN
    ...
    BEGIN
        ;WITH BatchSource AS (
         ...
         + A.cal_telefono5 + @emptyNumber),13)) as cal_telefono,
                CASE A.cal_status WHEN 7 THEN 1 ELSE A.cal_status END AS cal_status,
                A.cal_fechaDial,
            ...
            FROM ccoCallsOutSource A WITH (ROWLOCK, READPAST, INDEX(IX_ccoCallsOutSource_11))
            WHERE A.cam_id = @camp_id
              AND A.cal_status IN (0, 1, 7)
              AND NOT EXISTS (
                    SELECT 1
                    FROM ccoWorkingTable W WITH (NOLOCK)
                    WHERE W.callout_id = A.callout_id
              )
        )
        INSERT INTO ccoWorkingTable WITH (ROWLOCK)
        (
            callout_id, user_id, cam_id, cal_telefono, cal_status, cal_fechaDial, cal_keyw,
            iZonaHoraria, iZonaHoraria_verano, iZonaHoraria2, iZonaHoraria_verano2,
            iZonaHoraria3, iZonaHoraria_verano3, iZonaHoraria4, iZonaHoraria_verano4,
        ...
        IF @processed < @batchSize BREAK; -- No más registros
    END

    -- Update final (global, fuera del batch)
    UPDATE ccoCallsOutSource WITH (ROWLOCK, READPAST)
    SET cal_status = 3,
        dial_tels = @prioridad,
        nOcupado = 0,
        nNoContesta = 0,
...
'
    ...
    SET @process = 'ALTER procedure [dbo].[ccsp_ManualCallApplyTimeZoneRules] se quita la consulta sql dinámicas'
    SET @sql = 'ALTER procedure [dbo].[ccsp_ManualCallApplyTimeZoneRules]
@campid as int, @tel varchar(30) as
...
cam_id int,
cal_telefono varchar(30)collate SQL_Latin1_General_CP1_CI_AS,
izonahoraria int,
...
-- Validar si la zona del teléfono coincide con las zonas válidas
IF EXISTS (
    SELECT 1
    FROM #TimeZone
    WHERE cam_id = @campid
        AND (
            ((CASE WHEN @bIsDaylight = 1 THEN izonahoraria_verano ELSE izonahoraria END) & @iZonas) > 0
            OR (CASE WHEN @bIsDaylight = 1 THEN izonahoraria_verano ELSE izonahoraria END) = 0
            )
...
set nocount off'
    ...
    SET @process = 'ALTER PROCEDURE [dbo].[ccsp_OUTcheckTimeZone] se quita campo fecha ya que no se usa'
...
DECLARE @isSmsCamp BIT = CASE WHEN @campType = 7 THEN 1 ELSE 0 END;
  -- Obtener horarios legales
DECLARE @schLaw TABLE (
...
BEGIN
    ...
    SELECT
        h.horario_id,
        Descripcion,
        CASE WHEN HoraInicio > @hourStart THEN HoraInicio ELSE @hourStart END AS HoraInicio,
        CASE WHEN (HoraInicio > @hourStart OR (HoraInicio = @hourStart AND MinInicio >= @minStart)) THEN MinInicio ELSE @minStart END AS MinInicio,
        CASE WHEN HoraFin < @hourEnd THEN HoraFin ELSE @hourEnd END AS HoraFin,
        CASE WHEN (HoraFin < @hourEnd OR (HoraFin = @hourEnd AND MinFin <= @minEnd)) THEN MinFin ELSE @minEnd END AS MinFin,
    ...
    FROM (
        SELECT tz_id,
            DATEPART(HOUR, DATEADD(MINUTE, tz_offset * 60, @horaUniversal)) AS hora,
            DATEPART(MINUTE, DATEADD(MINUTE, tz_offset * 60, @horaUniversal)) AS minuto,
            DATEPART(WEEKDAY, DATEADD(MINUTE, tz_offset * 60, @horaUniversal)) AS dia
        FROM ccTimeZones
        ) zonas
...
BEGIN
    ...
    AS (
        SELECT
            DATEPART(HOUR, iDate) AS HoraInicio,
            DATEPART(MINUTE, iDate) AS MinInicio,
            DATEPART(HOUR, fDate) AS HoraFin,
            DATEPART(MINUTE, fDate) AS MinFin
        FROM ccSmsSchedules
        WHERE cam_id = @cam_id
            AND @dateNow BETWEEN DATEADD(HOUR, -12, iDate) AND DATEADD(HOUR, 12, fDate)
        ), daysch
    AS (
        SELECT
            CASE WHEN HoraInicio > @hourStart THEN HoraInicio ELSE @hourStart END AS HoraInicio,
