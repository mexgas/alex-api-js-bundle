...
'
	...
	SET @process = 'delete sp ccsp_VirtualAgents'
	SET @sql = '
	...
	SET @process = 'create sp ccsp_VirtualAgents'
	SET @sql = '
...
BEGIN
	...
	BEGIN
		SELECT
			...
			ISNULL(
				CASE 
					WHEN va.CampType = 0 THEN ci.IDArea -- Campa�a de entrada
					ELSE co.IDArea -- Campa�a de salida
				END,
			0) AS IDArea,
			CAST(ISNULL(
				CASE 
			...
			ISNULL(
				CASE 
					WHEN va.CampType = 0 THEN CAST(ci.Status AS BIT) -- Estado de la campa�a de entrada
					ELSE CAST(co.cam_procesando AS BIT) -- Estado de la campa�a de salida
				END, 0
			) AS IsActiveCampaign, -- Devuelve 1 o 0
			ISNULL(
				CASE 
	...
	ELSE IF @action = 2
	BEGIN
		-- Creaci�n de un nuevo agente virtual
		INSERT INTO dbo.ccVirtualAgent (
			nameAgent, 
			statusAgent, 
			createDateAgent, 
		...
		VALUES (
			@nameAgent, 
			@statusAgent, 
			GETDATE(), -- Fecha de creaci�n actual
			NULL -- latestUpdateDateAgent
		);
