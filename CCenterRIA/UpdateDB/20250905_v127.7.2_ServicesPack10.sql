...
sera necesario poner solo el fix es decir @version = 01 y ccsp_getVersion ''BDF'' se tendra que tener cuidado con las versiones ya que */
    ...
    --- BEGIN DMM ---
    SET @process = 'DROP ccsp_VirtualAgents'
    ...
    BEGIN
        ...
        ELSE IF (@action = 17)
        BEGIN
            SELECT va.quantumAgentId as AgentId, vav.quantumVoiceId as Voice
                FROM ccVirtualAgent AS va
                INNER JOIN ccVirtualAgentVoices AS vav 
                ON va.voice = vav.id
                WHERE va.idAgent = @idVirtualAgent;
        END
    END'     
...
END'
    ...
    SET @process = 'ALTER PROCEDURE [dbo].[ccsp_ConversationWASaveOut] Fix SaveMessageOut'
...
BEGIN
   ...
   DECLARE @StartedTransaction BIT = 0;
    ...
    BEGIN TRY
        ...
        ELSE
        BEGIN
            SAVE TRANSACTION SaveMessageFromConversationOut;
        END;
    ...
    BEGIN CATCH
        ...
        BEGIN
            IF @StartedTransaction = 1
                ROLLBACK TRAN;
            ELSE
                ROLLBACK TRAN SaveMessageFromConversationOut;
        END;
...
'
    ...
    SET @process = 'ALTER PROCEDURE [dbo].[ccsp_ConversationWASave]'
...
ELSE IF @action = 4 
BEGIN --save messages from conversation
    DECLARE @FirstMessageAgent DATETIME;
    DECLARE @exists INT;

    SELECT 
        @exists = 1, 
        @FirstMessageAgent = FirstMessageAgent
    FROM ccWhatsAppConversations A WITH (UPDLOCK, HOLDLOCK)
    WHERE A.conversationId = @conversationId;
    ...
    BEGIN TRY
        ...
        ELSE
        BEGIN
            SAVE TRANSACTION SaveMessageFromConversation;
        END;
    ...
    BEGIN CATCH
        ...
        BEGIN
            IF @StartedTransaction = 1
                ROLLBACK TRAN;
            ELSE
                ROLLBACK TRAN SaveMessageFromConversation;
        END;
