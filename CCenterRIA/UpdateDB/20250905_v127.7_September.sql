...
sera necesario poner solo el fix es decir @version = 01 y ccsp_getVersion ''BDF'' se tendra que tener cuidado con las versiones ya que */
	...
	SET @sql = '
	IF NOT EXISTS(SELECT 1 FROM sys.tables WHERE name = ''unassignAgentInfoTmp'')
	BEGIN
		CREATE TABLE unassignAgentInfoTmp (
			...
			CONSTRAINT FK_unassignAgentInfoTmp_ccUser FOREIGN KEY (userId) 
				REFERENCES dbo.ccUsers ([User_id]) 
				ON DELETE CASCADE 
		);
	END '
	EXEC(@sql)
...
END'
    ...
    SET @process = 'CW-9315, #6302 Validar SP  - Drop procedure ccsp_GalateaAdminCampaigns'
...
'
	EXEC(@sql)
		...
		SET @process = 'Se modifica ccsp_GalateaAdminUploadBLst para eliminar correctamente 1 numero cuando existe el numero con y sin calkey'
...
BEGIN
	--Check if phone number exists
			...
			BEGIN
					...
					IF(@isKolob = 1)
					BEGIN
						--Check if phone without HashKey exists
						IF EXISTS(SELECT 1 FROM dbo.ccListaNegra AS cln WHERE 
						cln.Hashtel = @hashPhone AND cln.idtipolista = @idtipolista AND HashKey IS NULL)
						BEGIN
							INSERT INTO cchistoriallistanegra (telefono, idtipomov, idtipolista)
...
END
	...
	DROP TABLE IF EXISTS #TempRawDataDelete;

	-- 1. Tabla Temporal para estructurar los bloques de salida
	CREATE TABLE #TempRawDataDelete (
		RowID INT IDENTITY(1,1) PRIMARY KEY,
		User_id INT,
		IdCampEsp INT,
		Tipo INT,
	...
	-- 2. Lógica de Negocio: Cruzar la tabla temporal de desasignación con los usuarios recibidos
	;WITH TargetUsers AS (
		SELECT DISTINCT CAST(Value AS INT) as User_id
		FROM dbo.fn_RIASplitDelimited(@userIds, '','')
		WHERE Value IS NOT NULL AND Value <> ''''
	...
	UserTotals AS (
		-- Calculamos el TotalTipo basándonos en lo que se está desasignando 
		-- filtrando solo por los usuarios del parámetro
		SELECT 
			T.userId, 
			(MAX(CASE WHEN T.tipo = 0 THEN 1 ELSE 0 END) + 
			 MAX(CASE WHEN T.tipo = 1 THEN 2 ELSE 0 END)) as TotalTipo
		FROM dbo.unassignAgentInfoTmp T
		INNER JOIN TargetUsers U ON T.userId = U.User_id
		GROUP BY T.userId
	)
	INSERT INTO #TempRawDataDelete (User_id, IdCampEsp, Tipo, Prioridad, Skill, TotalTipo)
	SELECT 
		T.userId,
		T.idcamp,
		T.tipo,
	...
	FROM dbo.unassignAgentInfoTmp T
	INNER JOIN TargetUsers U ON T.userId = U.User_id
	LEFT JOIN UserTotals UT ON T.userId = UT.userId
	ORDER BY T.userId, T.idcamp;

	-- 3. Generación de Segmentos concatenados
	SELECT 
		Groups.BlockID + 1 AS segmentId, 
		CAST(STUFF((
			SELECT '','' + 
	...
	FROM (
		SELECT DISTINCT (RowID - 1) / @RowsPerBlock AS BlockID
		FROM #TempRawDataDelete
	) Groups
	ORDER BY Groups.BlockID;

	DELETE T
		FROM dbo.unassignAgentInfoTmp AS T
		INNER JOIN dbo.fn_RIASplitDelimited(@userIds, '','') AS F
			ON T.userId = CAST(F.Value AS INT);

	DROP TABLE #TempRawDataDelete;
...
-----------------------------Begin Ulises Espinosa ------------------------------
 SET @process = 'Fix Pisition al transferir';

 SET @sql = 'CREATE OR ALTER PROCEDURE [dbo].[ccsp_RIAChecaLogin]
    @login             VARCHAR(40)
,   @password          VARCHAR(40)
,   @computer          VARCHAR(20)
...
----------------------------- BEGIN MAGV DEV3-1443 -----------------------------
  ...
  set @process = 'Delete ccsp_GalateaManageWG'
    ...
    EXEC(@sql)
        ...
        set @process = 'DEV3-1443 Create sp ccsp_GalateaManageWG se modifica el option 2, para insertar información en la nueva tabla unassignAgentInfoTmp,
		para que el desasignar realice correctamente la desasignación'
...
SET @process = ' obtener el último estado del agente desde ccLogAgentesDiaLas ccsp_GalateaAdminGetAgentCounters'
SET @sql = '

CREATE PROCEDURE [dbo].[ccsp_GalateaAdminGetAgentCounters]
@type AS INT,
@sup_id AS INT = 0,
@agent_id AS INT = 0,
@WG AS INT = 0,
...
CASE
WHEN p.publicIp IS NULL
OR p.publicIp = ''''
THEN ''000.000.000.000''
ELSE p.publicIp
END IP
FROM ccUsers u
...
SELECT
CASE
WHEN CurrentState.currentStatus IS NULL
...
FROM UserMaxFecha A
INNER JOIN ccLogAgentesDia B
ON A.User_id = B.User_id
AND A.fecha = B.fecha
) CurrentState
ON u.User_id = CurrentState.User_id
...
us.Nombres + '' '' + us.ApellidoPaterno + '' '' + us.ApellidoMaterno AS Name
FROM ccRIAWorkGroupUsers A
INNER JOIN ccUsers us
...
SELECT
CONVERT(INT, us.User_id) AS Id,
us.Username AS Username,
us.Name,
LastStateId =
CASE
WHEN B.currentStatus IS NULL OR B.currentStatus < 0 THEN 0
ELSE B.currentStatus
END
FROM wgAgt us
LEFT JOIN ccLogAgentesDiaLast B
...
SELECT
CAST(u.User_id AS INT) AS UserId,
CASE
WHEN CurrentState.currentStatus IS NULL
OR CurrentState.currentStatus < 0 THEN 0
ELSE CAST(CurrentState.currentStatus AS INT)
...
us.Login Username,
us.Nombres + '' '' + us.ApellidoPaterno + '' '' + us.ApellidoMaterno AS Name,
ISNULL(B.LastStateId,0) LastStateId,
ISNULL(c.publicIp,''0.0.0.0'') as [Ip]
FROM ccusers us
LEFT JOIN currentState B
ON us.User_id = B.User_id
LEFT JOIN ccposicion C
