			...
			BEGIN
				...
				BEGIN
					SELECT 
						...
						''{"type":0,"agent_id":"'' + ISNULL(v.quantumAgentId, '''') + ''",''+''"country":''+ISNULL(c.valor,0)+'',''+''"time_zone":0}'' AS data_api_quantum
					FROM 
					...
					OUTER APPLY
						(SELECT valor FROM ccSettings WHERE setting_id = 104) AS c
				END
			END
	 '
    EXEC(@sql)
