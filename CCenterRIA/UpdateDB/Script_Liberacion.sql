...
SET @process = ''
SET @sql = 'if exists (select * from sys.procedures where name = N''trsp_GetListaBorrarRespaldo'')
begin
	DROP PROCEDURE trsp_GetListaBorrarRespaldo;
end'
EXEC(@sql)
...
SET @process = ''
SET @sql = 'CREATE PROCEDURE [dbo].[trsp_GetListaBorrarRespaldo]
@cwIntegrated AS INT,
...
BEGIN
	...
	IF @cwIntegrated = 1
	BEGIN
		/* Se elimina codigo de tablas de grabadora stand alone */
		select null as grab_id, null as cal_id, null as Tipo_Llamada, null as status_audio WHERE 1 = 0
	END
...
if (datediff(ss, @upd_date, getdate()) > 300) begin
    ...
    begin
        ...
        declare @vop1 decimal(11,2)
        declare @vop2 decimal(11,2)
        declare @vop3 decimal(11,2)
        declare @vop4 decimal(11,2)
