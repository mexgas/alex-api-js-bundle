...
/****** Object:  Job [CW Delete old records]    Script Date: 04/05/2026 12:48:26 p. m. ******/
...
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Nuxiba]    Script Date: 04/05/2026 12:48:26 p. m. ******/
IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N''Nuxiba'' AND category_class=1)
BEGIN
EXEC @ReturnCode = msdb.dbo.sp_add_category @class=N''JOB'', @type=N''LOCAL'', @name=N''Nuxiba''
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N''CW Delete old records'', 
		...
		@owner_login_name=N''replication'', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Delete Call And catalog]    Script Date: 04/05/2026 12:48:27 p. m. ******/
...
INSERT INTO #sqlCmdDeleteOldRecords (sqlCmd, [status], isReplicated) 
VALUES (''''delete TOP (@batchSize) from ccAniRecordState where LastRefreshAt < @date'''', 0, 1);

INSERT INTO #sqlCmdDeleteOldRecords (sqlCmd, [status], isReplicated) 
VALUES (''''delete TOP (@batchSize) from ccAniPrefixState where LastRefreshAt < @date'''', 0, 1);

INSERT INTO #sqlCmdDeleteOldRecords (sqlCmd, [status], isReplicated) 
VALUES (''''delete TOP (@batchSize) from ccAniA3State where LastRefreshAt < @date'''', 0, 1);
...
DROP TABLE #ccoCallsOutSourceIds;'', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Delete SMS]    Script Date: 04/05/2026 12:48:27 p. m. ******/
...
'
    ...
    set @process = 'CREATE JOB CleanTableSmsRemesasMuñozDay'
    set @sql = 'USE [msdb]

if exists(select * from  [msdb].[dbo].[sysjobs] AS [sJOB] where [name]=N''CleanTableSmsRemesasMuñozDay'') begin
	EXEC msdb.dbo.sp_delete_job @job_name=N''CleanTableSmsRemesasMuñozDay'', @delete_unused_schedule=1
end
...
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Nuxiba]    Script Date: 04/09/2024 11:49:17 p. m. ******/
IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N''Nuxiba'' AND category_class=1)
BEGIN
EXEC @ReturnCode = msdb.dbo.sp_add_category @class=N''JOB'', @type=N''LOCAL'', @name=N''Nuxiba''
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
DECLARE @jobId BINARY(16)
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N''CleanTableSmsRemesasMuñozDay'', 
		@enabled=1, 
		@notify_level_eventlog=0, 
		@notify_level_email=0, 
		@notify_level_netsend=0, 
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N''truncate table SmsRemesasMuñozDay'', 
		@category_name=N''Nuxiba'', 
		@owner_login_name=N''replication'', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [ccKillList]    Script Date: 04/09/2024 11:49:18 p. m. ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N''ccKillList'', 
		@step_id=1, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
...
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N''CleanTableSmsRemesasMuñozDay'', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=8, 
...
EndSave:'
    ...
    set @process = 'CREATE JOB ccsp_ProcessDNCQueueJob'
    set @sql = 'USE [msdb]
...
EndSave:'
    ...
    set @process = 'CREATE JOB Jobccsp_PurgeAniStateOrphans'
    set @sql = 'USE [msdb]

if exists(select * from  [msdb].[dbo].[sysjobs] AS [sJOB] where [name]=N''Jobccsp_PurgeAniStateOrphans'') begin
    EXEC msdb.dbo.sp_delete_job @job_name=N''Jobccsp_PurgeAniStateOrphans'', @delete_unused_schedule=1
end
/****** Object:  Job [Jobccsp_PurgeAniStateOrphans]    Script Date: 04/05/2026 01:36:08 p. m. ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Nuxiba]    Script Date: 04/05/2026 01:36:09 p. m. ******/
IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N''Nuxiba'' AND category_class=1)
BEGIN
EXEC @ReturnCode = msdb.dbo.sp_add_category @class=N''JOB'', @type=N''LOCAL'', @name=N''Nuxiba''
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
DECLARE @jobId BINARY(16)
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N''Jobccsp_PurgeAniStateOrphans'', 
		@enabled=1, 
		@notify_level_eventlog=0, 
		@notify_level_email=0, 
		@notify_level_netsend=0, 
		...
		@owner_login_name=N''sa'', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [ccsp_PurgeAniStateOrphans]    Script Date: 04/05/2026 01:36:09 p. m. ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N''ccsp_PurgeAniStateOrphans'', 
		@step_id=1, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
		@on_success_step_id=0, 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N''TSQL'', 
		@command=N''exec ccsp_PurgeAniStateOrphans'', 
		@database_name=N''CCenterRIA'', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N''ccsp_PurgeAniStateOrphans'', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=1, 
		...
		@freq_relative_interval=0, 
		@freq_recurrence_factor=0, 
		@active_start_date=20260504, 
		@active_end_date=99991231, 
		@active_start_time=221000, 
		@active_end_time=235959, 
		@schedule_uid=N''5bc5bc87-8498-4d68-a074-4889e53e1ff5''
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobserver @job_id = @jobId, @server_name = N''(local)''
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
COMMIT TRANSACTION
...
'
    ...
    set @process = 'CREATE JOB NuxibaJobUpdateCalIdSurveyIVROptions'
    set @sql = 'USE [msdb]

if exists(select * from  [msdb].[dbo].[sysjobs] AS [sJOB] where [name]=N''NuxibaJobUpdateCalIdSurveyIVROptions'') begin
    EXEC msdb.dbo.sp_delete_job @job_name=N''NuxibaJobUpdateCalIdSurveyIVROptions'', @delete_unused_schedule=1
end
/****** Object:  Job [Jobccsp_PurgeAniStateOrphans]    Script Date: 04/05/2026 01:36:08 p. m. ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
...
DECLARE @jobId BINARY(16)
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N''NuxibaJobUpdateCalIdSurveyIVROptions'', 
		@enabled=1, 
		@notify_level_eventlog=0, 
		@notify_level_email=0, 
		@notify_level_netsend=0, 
		...
		@owner_login_name=N''sa'', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [NuxibaJobUpdateCalIdSurveyIVROptions]    Script Date: 10/05/2026 11:59:32 p. m. ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N''NuxibaJobUpdateCalIdSurveyIVROptions'', 
		@step_id=1, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
		@on_success_step_id=0, 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N''TSQL'', 
		@command=N''exec ccsp_DLRSaveDialResultBulk @Action=3'', 
		@database_name=N''CCenterRIA'', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N''NuxibaJobUpdateCalIdSurveyIVROptions'', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=8, 
		...
		@freq_relative_interval=0, 
		@freq_recurrence_factor=0, 
		@active_start_date=20260510, 
		@active_end_date=99991231, 
		@active_start_time=2000, 
		@active_end_time=232100
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobserver @job_id = @jobId, @server_name = N''(local)''
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
COMMIT TRANSACTION
