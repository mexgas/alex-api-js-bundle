...
if @Version_Actual = @Version -1 -- Aqui poner numero de nueva version
	...
	set @process = 'Create index RIA_GRABACION.IX_RIA_GRABACION_9 cw-969'
 	set @sql ='if not exists (select * from sys.indexes where name = N''IX_RIA_GRABACION_9'' and object_id = OBJECT_ID(N''RIA_GRABACION''))
    begin
        CREATE INDEX IX_RIA_GRABACION_9 ON RIA_GRABACION (ani)
    end
'
	EXEC(@sql)

	set @process = 'Create index RIA_GRABACIONCONSULTA.IX_RIA_GRABACIONCONSULTA_9 cw-969'
 	set @sql ='if not exists (select * from sys.indexes where name = N''IX_RIA_GRABACIONCONSULTA_9'' and object_id = OBJECT_ID(N''RIA_GRABACIONCONSULTA''))
    begin
        CREATE INDEX IX_RIA_GRABACIONCONSULTA_9 ON RIA_GRABACIONCONSULTA (ani);
    end'
	EXEC(@sql)


	set @process = 'ALTER SP -- trsp_AdmRecSearchANI cw-969'
...
END'
	...
	set @process = 'ALTER SP trsp_AdmRecSearchCalID -- cw-969'
