...
if @action in(1,6) begin--obtiene los nodos a insertar en BX
	if @option = 2 begin
		...
		--set @sql=''select x.grab_id,x.xmlString,baseX.Xname  from (
		--	select top('' + @top + '') grab_id, replace(replace(convert(nvarchar(max),node),''''{'''',''''&#123;''''),''''}'''',''''&#125;'''') xmlString
	 --	    ,node.value(''''(/R02/@CDATE)[1]'''',''''datetime'''') as dateNode			
		--	 from ria_RecNode A with(rowlock)  
		--	 where A.status ='' + cast(@status as nvarchar(max)) +'')X
		--	 left join ccBaseXDB baseX on baseX.serviceId=2  and X.dateNode between baseX.dateStart and isnull(baseX.dateEnd,getdate())
...
ALTER PROCEDURE [dbo].[ccsp_CleanNodeBaseX]
@option int
AS
BEGIN
