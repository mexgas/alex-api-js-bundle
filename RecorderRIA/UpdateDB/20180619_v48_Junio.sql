...
EXEC(@sql)
	...
	set @sql='if not exists(select * from sys.triggers where name = N''MSmerge_tr_altertable'') begin
			if not exists (select * from sys.columns where name = N''prefijo'' and Object_ID = Object_ID(N''ccInbound'')) begin
			    alter table ccInbound ADD prefijo varchar(40) null
			end
			if not exists (select * from sys.columns where name = N''prefijo'' and Object_ID = Object_ID(N''ccCamps'')) begin
