...
-----------------------------------------------------------------------------------------------------------------------
set @process = 'Create table RECORDERRIA_RECORDINGEVALUATION'
		set @Sql = '
			IF not exists
			(
			SELECT *
			FROM INFORMATION_SCHEMA.TABLES
			WHERE TABLE_SCHEMA = ''dbo'' AND TABLE_NAME = ''RECORDERRIA_RECORDINGEVALUATION''
			)
			BEGIN
				  CREATE TABLE RECORDERRIA_RECORDINGEVALUATION  
				   (idRecordingEvaluation int PRIMARY KEY NOT NULL IDENTITY(1,1),  
				   grab_id int NOT NULL,  
				   userAdmin VARCHAR(50) NOT NULL,    
				   idFormat int NOT NULL,  
				   totalPoints int NOT NULL,
				   generalQualification int NOT NULL,
				   nameAdmin varchar(50) NOT NULL,
				   nameSupervisor  varchar(50) NOT NULL,
				   userSupervisor  varchar(50) NOT NULL,
		...
		EXEC(@Sql)
--------------------------------------------------------------------------------------------------
		set @process = 'Create table RECORDERRIA_ANSWERSOFQUESTIONSEVALUATION'
		set @Sql = '
			IF not exists
			(
			SELECT *
			FROM INFORMATION_SCHEMA.TABLES
			WHERE TABLE_SCHEMA = ''dbo'' AND TABLE_NAME = ''RECORDERRIA_ANSWERSOFQUESTIONSEVALUATION''
			)
			BEGIN
				  CREATE TABLE RECORDERRIA_ANSWERSOFQUESTIONSEVALUATION  
				   (idAnswerQuestions int PRIMARY KEY NOT NULL IDENTITY(1,1),
				   idRecordingEvaluation int NOT NULL,
...
-----------------------------------------------------------
	...
	set @Sql = 'if exists (select * from sys.procedures where name = N''ccsp_GalateaRecordingEvaluation'')
            begin
          DROP PROCEDURE ccsp_GalateaRecordingEvaluation;
            end'
	EXEC(@Sql)
	set @process = 'Create sp ccsp_GalateaRecordingEvaluation'
	...
	EXEC(@Sql)
	----------------------------------------------------
	set @process = 'Delete if exist sp trsp_InsertRecNode'
	set @Sql = 'if exists (select * from sys.procedures where name = N''trsp_InsertRecNode'')
            begin
          DROP PROCEDURE trsp_InsertRecNode;
            end'
	EXEC(@Sql)
	set @process = 'Create sp trsp_InsertRecNode'
	set @Sql = 'CREATE PROCEDURE [dbo].[trsp_InsertRecNode] @grabId INT, @type INT = 0, @rateEvaluationFormatKolob BIT = 0 
				AS
				BEGIN
