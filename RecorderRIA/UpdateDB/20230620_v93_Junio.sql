...
----------------------------------------- BEGIN 125.20230425.0.6 -----------------------------------------
		set @process = 'CW-7941 Alter SP ccspGalatea_Finder @action=2 se cambia la tabla principal grab(CTE) en lugar RIA_MARCAS'
...
END'
		EXEC(@sql)
----------------------------------------- END 125.20230425.0.6 -----------------------------------------


      
      
      -------------------------------------------- Begin Jesus Gallardo hotfix/25.20230719.0.7 -------------------------------------------------------------------------------
	set @process = 'DEV1-435 Alter Sp trsp_InsertRecNode se cambia para si el nodo es nulo en actualizacion este modify en lugar insertar un nodo null'
...
END'
	...
	-------------------------------------------- Begin Jesus Gallardo hotfix/25.20230719.0.9 -------------------------------------------------------------------------------
      SET @process = 'DEV1-440 alter SP ccspGalatea_Finder CW-7941 y DEV1-239_Send_cal_ids_list_to_front, se quita @action 9,10 y 11 se cambia @action=2'
        SET @sql = 'ALTER PROCEDURE [dbo].[ccspGalatea_Finder] 
@action int,
@grabIds varchar(max)=null,
@grabId int =null,
