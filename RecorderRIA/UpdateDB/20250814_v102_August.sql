...
begin
    ...
    SET @process = 'K070199 - Drop SP trsp_InsertRecNodeGrabIds'

    SET @sql = 'if exists (select * from sys.procedures where name = N''trsp_InsertRecNodeGrabIds'')
    BEGIN
        DROP PROCEDURE dbo.trsp_InsertRecNodeGrabIds;
    END'
    EXEC(@sql);

	 SET @process = 'K070199 - CREATE SP trsp_InsertRecNodeGrabIds'

    SET @sql = '
        CREATE PROCEDURE trsp_InsertRecNodeGrabIds
