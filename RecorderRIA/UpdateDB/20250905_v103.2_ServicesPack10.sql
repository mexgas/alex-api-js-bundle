...
begin
	...
	-----------------------------------BEGIN Sercvices Pack 10 ---------------------------------------

    SET @process = 'ALTER PROCEDURE [dbo].[trsp_InsertRecNodeGrabIds]'
...
BEGIN
    ...
    C32 VARCHAR(250)    ------> Fecha y Hora Final
);
...
;with GrabacionSource AS (
    ...
    rec.cal_id, rec.finicio,rec.ffin, rec.extra_info, rec.extra_info2, rec.video, 
    ...
    rec.cal_id, rec.finicio,rec.ffin, rec.extra_info, rec.extra_info2, NULL AS video,
    isnull(E.total,0) AS total_forma
    ,calif_id,cam_id,duracion,ani,dni,cal_key,id_repositorio,califSub_id,age_id
    ,cal_extension, rec.virtualAgentId
...
case 
    when rec.calif_id<=0 then ''N/A'' 
    when rec.tipo_llamada = 1 and rec.virtualAgentId=0  then e.description  
    when rec.tipo_llamada = 1 and rec.virtualAgentId>0  then tcia.Name_cal  
    when rec.tipo_llamada = 2 and outb.CampType = 9 then tcia.Name_cal
    when rec.tipo_llamada = 2 then eOut.Description 
end  AS ''@C20'',
rec.calif_id AS ''@C21'',
rec.video AS ''@C22'',
...
CASE WHEN rec.ffin IS NULL THEN '''' ELSE CONVERT(VARCHAR(23), rec.ffin, 126)END AS ''@C32''
...
LEFT JOIN cctipoCalif_IA AS tcia ON tcia.calif_id = rec.calif_id --AND rec.tipo_llamada = 2
