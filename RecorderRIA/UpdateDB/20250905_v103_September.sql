...
END'
	...
	SET @sql = '
		IF EXISTS (SELECT * FROM sys.procedures WHERE name = N''trsp_InsertRecNodeGrabIds'')
		BEGIN
 ...
 set @process = 'Se Hace el cambio para regresar la calificacion con los cambios de producto'
     set @sql='    CREATE PROCEDURE trsp_InsertRecNodeGrabIds
        ...
        if @rateEvaluationFormatKolob = 0 begin
            ...
            SELECT 
                A.grab_id,
                CAST(ROUND(AVG((CAST(A.totalPoints AS float) / NULLIF(CAST(A.generalQualification AS float), 0)) * 100), 0) AS INT)
            FROM RECORDERRIA_RECORDINGEVALUATION A
    ...
    SET @process = 'CW-9760 DROP TABLE ccBaseXDB;'
    SET @sql = '
       IF EXISTS (
        SELECT 1
        FROM sys.columns c
