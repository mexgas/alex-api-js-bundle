...
declare @settingBD varchar(500)
	select @settingBD = par_valor from TREC_PARAMETROS where par_id = 66
...
END'
	...
	SET @process = 'Drop Procedure sp_ExecPendingSubscriptions_CCRecorderRIA'
    SET @sql = '
	if exists (select * from sys.procedures where name = N''sp_ExecPendingSubscriptions_CCRecorderRIA'')
	begin
		DROP PROCEDURE sp_ExecPendingSubscriptions_CCRecorderRIA
	end'
    EXEC(@sql)


	SET @process = 'Create Procedure sp_ExecPendingSubscriptions_CCRecorderRIA'
    SET @sql = 'CREATE PROCEDURE [dbo].[sp_ExecPendingSubscriptions_CCRecorderRIA]
AS
BEGIN
...
END'
	...
	set @process = 'Create Job CW_Tran_Replication_CCRecorderRIA'
	set @Sql='USE [msdb]
if exists( select * from msdb.dbo.sysjobs where name=''CW_Tran_Replication_CCRecorderRIA'')
EXEC msdb.dbo.sp_delete_job @job_name=N''CW_Tran_Replication_CCRecorderRIA'', @delete_unused_schedule=1
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N''CW_Tran_Replication_CCRecorderRIA'', 
		@enabled=1, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N''CW_Tran_Replication_CCRecorderRIA'', 
		@step_id=1, 
		...
		@command=N''
exec sp_ExecPendingSubscriptions_CCRecorderRIA
