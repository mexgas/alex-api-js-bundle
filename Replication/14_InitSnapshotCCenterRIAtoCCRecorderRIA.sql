...
if @Version_Actual >= @Version
 begin
	...
	IF NOT EXISTS (SELECT * FROM dbo.sysobjects WHERE [name] = ''migrationAVRS'')
	BEGIN
		CREATE TABLE [dbo].[migrationAVRS](
		[id] [int] NOT NULL,
		...
		insert into migrationAVRS values (100 , ''SpecialAVRS'', 0, '''', '''', '''')
		insert into migrationAVRS values (101 , ''AVRSCampEsp'', 0, '''', '''', '''')
		insert into migrationAVRS values (102 , ''AVRSGraphs'', 0, '''', '''', '''')
		insert into migrationAVRS values (103 , ''AVRSSettings'', 0, '''', '''', '''')
		insert into migrationAVRS values (104 , ''ConversationMail'', 0, '''', '''', '''')
	END
	ELSE BEGIN
		insert into migrationAVRS values (104 , ''ConversationMail'', 0, '''', '''', '''')
	END'
...
/****** Object:  Job [AVRS Merge Replication]    Script Date: 08/08/2013 07:59:24 ******/
IF  EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = N''AVRS Merge Replication'')
	EXEC msdb.dbo.sp_delete_job @job_name=N''AVRS Merge Replication'', @delete_unused_schedule=1
	
/****** Object:  Job [AVRS Merge Replication]    Script Date: 07/01/2013 17:49:50 ******/
BEGIN TRANSACTION
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N''AVRS Merge Replication'', 
		...
		@command=N''if (select count(*) from migrationAVRS  with (nolock) where id >= 100 and [dateStart] = convert(datetime ,''''jan 1 1900'''') and [dateEnd] = convert(datetime ,''''jan 1 1900'''')) > 0
begin
	declare @id int
	...
	if @id > 100
		begin
			...
			if (select [dateStart] from migrationAVRS  with (nolock) where id = @tempId) <> convert(datetime ,''''jan 1 1900'''') 
			begin
				if exists(select *
	...
	if (select [dateStart] from migrationAVRS  with (nolock) where id = @id) = convert(datetime ,''''jan 1 1900'''') 
	begin 
...
if exists (select * from migrationAVRS where id = 104 and [dateStart] <> convert(datetime ,''''jan 1 1900'''') and [dateEnd] = convert(datetime ,''''jan 1 1900''''))
		begin
			if (select count(*)	
			from distribution.dbo.MSsnapshot_history a ,distribution.dbo.MSsnapshot_agents b 	
			...
			and runstatus = 2
			and publication = ''''AVRSSettings'''') = 1
			begin
