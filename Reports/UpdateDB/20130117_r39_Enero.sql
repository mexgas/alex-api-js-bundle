...
Descripcion: 
	...
	Se elimina la Foreign Key FK_ccoDialerCamp_ccoDialers de la tabla ccoDialerCamp
	Se crea el indice IX_ccoLogDials_7 en la tabla ccoLogDials
	Se crea el indice IX_ccoCallsOut_7 en la tabla ccoCallsOut
	Se crean los indices IX_ccPosicionEspecialidad1, IX_ccPosicionEspecialidad2 y IX_ccPosicionEspecialidad3 en la tabla ccPosicionEspecialidad
	Se crean los indices IX_ccPosicionCamps1, IX_ccPosicionCamps2, IX_ccPosicionCamps3 en la tabla cPosicionCamps
	Se inserta campo en la tabla exportReports para ccspIVRInsert
	Se insertan registros en la tabla exportReports para el paso de informacion de ccPosicionEspecialidad y ccPosicionCamps
	Se modifica la tabla exportReports para paso del parametro country_id de la tabla cstotipollamada
	Se modifica el campo job_id en la tabla exportReports para IVRStructure
	Se crea el Job NuxibaReportsMaintenancePlan como plan de mantenimiento a la BD ccReports

Version requerida: 38
*/
set nocount on
...
---------------- inicio SCRIPT @Sql ----------------
		...
		set @process = 'ccoDialerCamp - Alter Table'
		set @Sql='IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N''[dbo].[FK_ccoDialerCamp_ccoDialers]'') AND parent_object_id = OBJECT_ID(N''[dbo].[ccoDialerCamp]''))
ALTER TABLE [dbo].[ccoDialerCamp] DROP CONSTRAINT [FK_ccoDialerCamp_ccoDialers]'
	
	EXEC(@Sql)

		set @process = 'IX_ccoLogDials_7 - Create Index'
		set @Sql='IF  EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N''[dbo].[ccoLogDials]'') AND name = N''IX_ccoLogDials_7'')
DROP INDEX [IX_ccoLogDials_7] ON [dbo].[ccoLogDials] WITH ( ONLINE = OFF )

CREATE NONCLUSTERED INDEX [IX_ccoLogDials_7] ON [dbo].[ccoLogDials] 
(
	[callout_id] ASC,
	[Telefono] ASC,
	[cal_id] ASC,
	...
	EXEC(@Sql)
		...
		set @process = 'IX_ccoCallsOut_7 - Create Index'
		set @Sql='IF  EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N''[dbo].[ccoCallsOut]'') AND name = N''IX_ccoCallsOut_7'')
DROP INDEX [IX_ccoCallsOut_7] ON [dbo].[ccoCallsOut] WITH ( ONLINE = OFF )
...
(
	...
	EXEC(@Sql)
		...
		set @process = 'IX_ccPosicionEspecialidad1 - Create Index'
		set @Sql='CREATE NONCLUSTERED INDEX [IX_ccPosicionEspecialidad1] ON [dbo].[ccPosicionEspecialidad] 
(
	[Inbound_id] ASC,
	[User_id] ASC,
	[Tipo] ASC,
	...
	EXEC(@Sql)
		...
		set @process = 'IX_ccPosicionEspecialidad2 - Create Index'
		set @Sql='CREATE NONCLUSTERED INDEX [IX_ccPosicionEspecialidad2] ON [dbo].[ccPosicionEspecialidad] 
(
	[Inbound_id] ASC,
	[Tipo] ASC,
	[Fecha] ASC
	...
	EXEC(@Sql)
		...
		set @process = 'IX_ccPosicionEspecialidad3 - Create Index'
		set @Sql='CREATE NONCLUSTERED INDEX [IX_ccPosicionEspecialidad3] ON [dbo].[ccPosicionEspecialidad] 
(
	[Inbound_id] ASC,
	[User_id] ASC,
	[Fecha] ASC
	...
	EXEC(@Sql)
		...
		set @process = 'IX_ccPosicionCamps1 - Create Index'
		set @Sql='CREATE NONCLUSTERED INDEX [IX_ccPosicionCamps1] ON [dbo].[ccPosicionCamps] 
(
	[cam_id] ASC,
	[User_id] ASC,
	[Tipo] ASC,
	...
	EXEC(@Sql)
		...
		set @process = 'IX_ccPosicionCamps2 - Create Index'
		set @Sql='CREATE NONCLUSTERED INDEX [IX_ccPosicionCamps2] ON [dbo].[ccPosicionCamps] 
(
	[cam_id] ASC,
	[Tipo] ASC,
	[Fecha] ASC
	...
	EXEC(@Sql)
		...
		set @process = 'IX_ccPosicionCamps3 - Create Index'
		set @Sql='CREATE NONCLUSTERED INDEX [IX_ccPosicionCamps3] ON [dbo].[ccPosicionCamps] 
(
	[cam_id] ASC,
	[User_id] ASC,
	[Fecha] ASC
	...
	EXEC(@Sql)
		...
		set @process = 'exportReports - Insert(1)'
		set @Sql = 'insert into exportReports 
values(129,3,60,''ccspIVRInsert'',''4'',''1'','''',0)'

    EXEC(@Sql)

		set @process = 'exportReports - Insert(2)'
		set @Sql='insert into exportReports
values(15,2,10,''ccPosicionEspecialidad'','''',''Fecha'',''Inbound_id,User_id,Tipo,Fecha'',60)'
	
	EXEC(@Sql)

		set @process = 'exportReports - Insert(3)'
		set @Sql='insert into exportReports
values(16,2,10,''ccPosicionCamps'','''',''Fecha'',''cam_id,User_id,Tipo,Fecha'',60)'
	
	EXEC(@Sql)

		set @process = 'exportReports - Update(1)'
		set @Sql='DECLARE @var1 varchar(max)

SELECT @var1 = ''tipoLlamada_id,descrip,longitud,prefijo,country_id'' FROM exportReports WHERE jobId = 213

UPDATE exportReports SET cols = @var1 WHERE jobId = 213'
	
	EXEC(@Sql)	

		set @process = 'exportReports - Update(2)'
		set @Sql='update exportReports
set jobid = 228
where jobid = 227
and tableN = ''IVRStructure'''
	
	EXEC(@Sql)

		set @process = 'NuxibaReportsMaintenancePlan - Delete Job'
		set @Sql='USE [msdb]
/****** Object:  Job [NuxibaReportsMaintenancePlan]    Script Date: 01/17/2013 17:21:24 ******/
...
EXEC msdb.dbo.sp_delete_job @job_name=N''NuxibaReportsMaintenancePlan'', @delete_unused_schedule=1'
	...
	EXEC(@Sql)
		...
		set @process = 'NuxibaReportsMaintenancePlan - Create Job'
		set @Sql ='USE [msdb]
...
ALTER INDEX [PK__ccRIACat_WorkGro__546180BB] ON [dbo].[ccRIACat_WorkGroup] REORGANIZE WITH ( LOB_COMPACTION = ON )
...
ALTER INDEX [PK__ccRIACat_WorkGro__546180BB] ON [dbo].[ccRIACat_WorkGroup] REBUILD WITH ( PAD_INDEX  = OFF, STATISTICS_NORECOMPUTE  = OFF, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON, SORT_IN_TEMPDB = OFF, ONLINE = OFF )
