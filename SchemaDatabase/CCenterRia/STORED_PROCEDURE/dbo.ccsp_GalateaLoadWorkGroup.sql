...
if @option =1 --Obtiene elementos de un area
begin
	if not exists(select * from ccRIACat_Areas where IDArea = @areaId)
	begin
		select @agentes as agents, @admins as admins, @campsIn as campsIn, @campsOut as campsOut, @wgs as idsWg
	 return (0)
	end
	select @agentes = coalesce(@agentes + ',', '') +  convert(varchar(12),user_id)
	from ccUsers where TipoUser_id=1 and IdArea = @areaId
	order by user_id

	select @admins = coalesce(@admins + ',', '') +  convert(varchar(12),user_id)
	from ccUsers where TipoUser_id > 1 and IdArea = @areaId
	order by user_id

	select @campsIn = coalesce(@campsIn + ',', '') +  convert(varchar(12),inbound_id)
	from ccInbound where IdArea = @areaId
	order by inbound_id

	select @campsOut = coalesce(@campsOut + ',', '') +  convert(varchar(12),cam_id)
	from ccCamps where IdArea = @areaId
	order by cam_id

	select @wgs = coalesce(@wgs + ',', '') +  convert(varchar(12),IDWG)
	from ccRIAAreaWorkGroup where IDArea = @areaId
	order by IDWG
...
if @option =2 --Obtiene las relaciones de los WG de una area
begin
	SELECT 
