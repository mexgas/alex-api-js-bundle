CREATE procedure ccsp_GetNotReadyTimesxTipo
@user_id int = 0,
@idTipo int
AS
...
sum(tStatus) as total, l.tiponotready_id
...
and  fecha between @fStart and @fEnd
and (user_id = @user_id or @user_id = 0)
group by t.descripcion, l.tiponotready_id