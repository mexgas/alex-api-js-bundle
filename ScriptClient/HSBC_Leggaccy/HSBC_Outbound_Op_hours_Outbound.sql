...
CREATE PROCEDURE [dbo].[HSBC_Outbound_Op_hours_Outbound ]
...
BEGIN
	...
	SELECT CONVERT(DATE, [cal_inicio]) AS date
	     ...
	     , cam_id FROM ccoCallsOut with(nolock)
		 where cal_Inicio between @from and @to
	GROUP BY CONVERT(DATE, [cal_inicio])
	       , cam_id
	)

	select A.dateDay,A.camId,sum(A.timeAgent) +isnull(sum(B.timeDialog),0) as [Op_hours_Outbound] from userIdCam A
