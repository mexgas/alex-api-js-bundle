...
if @actualVersion = @version - 1
	...
	set @process = 'drop index IX_ccoDialers on ccoDialers ---------'
	set @Sql= 'if exists (select * from sys.indexes where name = N''IX_ccoDialers'' and object_id = OBJECT_ID(N''ccoDialers''))
	begin
		drop index IX_ccoDialers on ccoDialers
	end
	'
	EXEC(@Sql)

	set @process = 'drop index IX_ccoDialers_I on ccoDialers ---------'
	set @Sql= 'if exists (select * from sys.indexes where name = N''IX_ccoDialers_I'' and object_id = OBJECT_ID(N''ccoDialers''))
	begin
...
END	'
	...
	set @Sql= 'alter table ccodialers alter column Puerto int 	'
	EXEC(@Sql)
...
END'
	...
	set @Sql= '	if not exists(SELECT * FROM sys.indexes WHERE [name] = N''IX_ccoDialers'' AND object_id = OBJECT_ID(N''ccoDialers''))
		begin
	...
	set @Sql= 'if not exists(SELECT * FROM sys.indexes WHERE [name] = N''IX_ccoDialers_I'' AND object_id = OBJECT_ID(N''ccoDialers''))
		begin
			CREATE NONCLUSTERED INDEX [IX_ccoDialers_I] ON [dbo].[ccoDialers] 
			(
				[dialer_id] ASC,
				[provedor_id] ASC
			)
	...
	set @Sql= 'if not exists(select * from ccSettings where setting_id=35)
	insert into ccSettings (setting_id,valor,descripcion,Status,Tipo) values(35,'''',''hostname\UserWindow|passWindow|userSQL|passSQL|hostname'',1,''X'')'
	EXEC(@Sql)


	set @process = 'ALTER PROCEDURE [dbo].[ccspRepInEffectiveness] -----------'
...
if @action = 1 
	...
	left join ccusers u with(nolock) on (u.user_id = #IVRLlamadas.user_id)
	...
	set @process = 'ALTER procedure [dbo].[ccsp_getVersion]'
