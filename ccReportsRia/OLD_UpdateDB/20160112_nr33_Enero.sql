...
set columns = ''inboundId|inbound|dnisId|max(DniNumber):DniNumber|dnis|workgroupId|workgroup|areaId|area|sum([ntotalin]):ntotalin|sum([nxfer]):nxfer|sum([nabndque]):nabndque|sum([nxferque]):nxferque|sum([nnoxfer]):nnoxfer|MAX([tquemax]):tquemax|sum([tque]):tque|sum([nque]):nque|sum([nanswer]):nanswer|sum([nnoanswer]):nnoanswer|sum([nlost]):nlost|sum([nabndxferincall]):nabndxferincall|sum([nabndringincall]):nabndringincall|sum([nabnddialogincall]):nabnddialogincall|AVG([postot]):postot|AVG([postime]):postime|min([SLP1]):SLP1|min([SLP2]):SLP2|ISNULL(sum(tque)/ NULLIF(sum(nque)_ 0)_ 0):avg|ISNULL(SUM(SLP1) * 100/ NULLIF(SUM(SLP2)_ 0)_ 0):SL|sum([nMoh]):nMoh|sum([nWHag]):nWHag|sum([nWHcl]):nWHcl''
where id = 3020'
	EXEC(@Sql)

	set @process = 'alter table RepInCalls -- DniNumber'
	set @Sql= 'if not exists (select * from sys.columns where name = N''DniNumber'' and Object_ID = Object_ID(N''RepInCalls''))
	begin
		alter table RepInCalls add DniNumber varchar(50)
	end
	'
	EXEC(@Sql)


	set @process = 'update GroupByReports -- 3020'
	set @Sql= 'update GroupByReports 
set columns = ''inboundId|inbound|dnisId|max(DniNumber):DniNumber|dnis|workgroupId|workgroup|areaId|area|sum([ntotalin]):ntotalin|sum([nxfer]):nxfer|sum([nabndque]):nabndque|sum([nxferque]):nxferque|sum([nnoxfer]):nnoxfer|MAX([tquemax]):tquemax|sum([tque]):tque|sum([nque]):nque|sum([nanswer]):nanswer|sum([nnoanswer]):nnoanswer|sum([nlost]):nlost|sum([nabndxferincall]):nabndxferincall|sum([nabndringincall]):nabndringincall|sum([nabnddialogincall]):nabnddialogincall|AVG([postot]):postot|AVG([postime]):postime|min([SLP1]):SLP1|min([SLP2]):SLP2|ISNULL(sum(tque)/ NULLIF(sum(nque)_ 0)_ 0):avg|ISNULL(SUM(SLP1) * 100/ NULLIF(SUM(SLP2) _0)_ 0):SL|sum([nMoh]):nMoh|sum([nWHag]):nWHag|sum([nWHcl]):nWHcl''
where id = 3020	'
	EXEC(@Sql)


	set @process = 'ALTER PROCEDURE [dbo].[ccspRepInCalls] ---------'
