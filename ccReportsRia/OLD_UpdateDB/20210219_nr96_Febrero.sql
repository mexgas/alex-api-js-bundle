...
BEGIN
	...
	BEGIN TRY
		...
		SET @process = 'para validacion de la facturacion'
		SET @sql = 'if exists (select * from sys.procedures where name = N''ExportAnwsAndXfers'')
            begin
          DROP PROCEDURE ExportAnwsAndXfers;
            end'
		EXEC(@sql)
		
		SET @process = ' para validacion de la facturacion'
		SET @sql = 'Create PROCEDURE [dbo].[ExportAnwsAndXfers]
@type as tinyint,
...
'
		...
		SET @process = 'se cambia el calculo del tiempo de dialogo'
...
END'
		...
		SET @process = 'se cambia la forma de calculo de reporte de canceladas y xfers'
