...
set @process = 'Alter table - RepIVRSurveys'
	set @sql='
			update PivotReports set complementColumns=''date|userId|login|scriptId|surveyId|survey|calId|calKey|campaignId|inboundId|clientPhoneNumber|campACDDescription'' where id=6050'
	EXEC(@sql)
...
--inserto tiempo READY y NOT READY
	 ...
	 select 
	 	...
	 	from ccLogAgentesDia A
	 	inner JOIN #tempFechasR B ON A.fecha=B.fecha and A.User_id=B.id WHERE currentStatus in (1,2,3)	
	...
	SELECT
		   ...
		   where rowAgentInformation in
				 (select rowAgentInformation from #tempRepAgentGI temp GROUP BY temp.rowAgentInformation,temp.[UserId] HAVING Count(*) > 1 )	
