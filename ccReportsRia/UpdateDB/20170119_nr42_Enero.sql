...
if @actualVersion = @version - 1 begin
	...
	CREATE PROCEDURE [ccspRepDetailAgent]
		...
		if @action=1 begin
			...
			set @dateNow=getdate()
			set @interval=15
			declare @var as varchar = (select valor from ccsettings where setting_id = 39)
	...
	set @process = 'RepDialingAgent - filtros fecha y seleccion'
	set @Sql= '
	if not exists(select * from ReportsFiltersMenus where idReport = 2080)
		begin
			insert ReportsFiltersMenus (idReport, filterMenuName) values(2080, N''date''), (2080, N''filterby'')
		end
	'
	EXEC(@sql)

	set @process = 'RepDialingAgent - filtro usuario'
	set @Sql= '
	if not exists(select * from ReportsFilters where id=2080)
		begin
