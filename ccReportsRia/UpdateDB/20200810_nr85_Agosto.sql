...
BEGIN
	...
	BEGIN TRY
		...
		set @process = 'CW-4273 Insertar columna grabId en ccUsRepOutCallsDetailers'
		set @sql = '
		if not exists (select * from INFORMATION_SCHEMA.COLUMNS where COLUMN_NAME = ''grabId'' and TABLE_NAME = ''ccUsRepOutCallsDetailers'') begin
			alter table RepOutCallsDetail add grabId int null
		end
		'
		EXEC(@sql)

		set @process = 'CW-4273 Insertar columna grabId en ccUsRepInCallsDetailers'
		set @sql = '
		if not exists (select * from INFORMATION_SCHEMA.COLUMNS where COLUMN_NAME = ''grabId'' and TABLE_NAME = ''ccUsRepInCallsDetailers'') begin
		...
		set @sql = '
		ALTER PROCEDURE [dbo].[ccspRepInCallsDetail] @action AS TINYINT, @from AS DATETIME = NULL, @to AS DATETIME = NULL
		AS
