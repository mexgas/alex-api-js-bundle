...
BEGIN
	...
	set @sql = '
		...
		if not exists (select * from sys.columns where name = N''previewTime'' and Object_ID = Object_ID(N''RepOutDialDetail''))
			begin
				alter table RepOutDialDetail add previewTime smallint not null default 0
			end
		if not exists (select * from sys.columns where name = N''login'' and Object_ID = Object_ID(N''RepOutDialDetail''))
			begin
				alter table RepOutDialDetail add login varchar(40) not null default ''''
			end
	'
	EXEC(@sql)
