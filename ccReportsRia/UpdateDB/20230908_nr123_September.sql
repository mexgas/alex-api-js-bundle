...
BEGIN
	...
	and not exists (select * from sys.columns where name = N''incomingNumber'' and Object_ID = Object_ID(N''RepCallXfer''))
		begin
	...
	SET @sql = 'CREATE PROCEDURE [dbo].[ccspRepCallXfer]
@action as tinyint,
@from AS datetime = null,
@to AS datetime = null
AS
...
begin
	...
	insert RepCallXfer 
	(date,callid,CallTypes,Agent,xfertype,destination,timebeforexfer,timeafterxfer,startDate,endDate,Origin,TotalTimeDuration,TipoTel,userId,incomingNumber)
