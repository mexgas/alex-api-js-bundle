...
FROM clt
    ...
    ------------------------------------ End Gaby ---------------------------------------------------------------------
	------------------------------------Begin Frida--------------------------------------------------------------------
	SET @process = 'CW-9251 drop sp ccspRepCatalogos'
    SET @sql = 'if exists (select * from sys.procedures where name = N''ccspRepCatalogos'')
    begin
        DROP PROCEDURE ccspRepCatalogos
    end';
    EXEC(@sql);
	SET @process = 'CW-9251 create sp ccspRepCatalogos'
    SET @sql = '

	CREATE PROCEDURE ccspRepCatalogos
	@type as tinyint,
	...
	if @type = 6 	begin
		if @userId <> 0 begin
				...
				if exists(select 1 from @tempwork) and exists (select 1 from ccRIAWorkGroupUsers where IDWG in (select IDWG from @tempwork)) 
				begin
					...
					select 0 id, ''N/A'' as description,  ''userId'' as dbcolumn
				end
				return
			end
