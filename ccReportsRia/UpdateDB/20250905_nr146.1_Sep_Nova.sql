...
END'
    ...
    SET @sql = 'IF EXISTS (SELECT 1 FROM sys.indexes WHERE name = ''IX_RepACDChats_date''AND object_id = OBJECT_ID(''RepACDChats'') ) BEGIN DROP INDEX IX_RepACDChats_date ON RepACDChats END
...
'
    ...
    SET @sql = 'UPDATE GroupByReports set columns = ''max([areaId]):areaId|max([area]):area|workgroupId|max([workgroup]):workgroup|campaignId|max([campaign]):campaign|userId|max([user]):user|sum([ntotal]):ntotal|sum([nxfer]):nxfer|sum([nnoagent]):nnoagent|sum([nanswer]):nanswer|sum([nnoanswer]):nnoanswer|sum([nlost]):nlost|sum([nabndxfer]):nabndxfer|sum([nabndring]):nabndring|sum([nabnddialog]):nabnddialog|sum([postot]):postot|sum([postime]):postime|sum([nhangup]):nhangup|sum([tatencion]):tatencion'' WHERE id = 4030;'
