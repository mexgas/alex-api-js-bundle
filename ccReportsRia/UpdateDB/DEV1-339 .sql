...
END'
	...
	insert into pivotReports values(2100,''descripcion_time'',''date|login|user|sessionTime|loginMktTime|logoutMktTime|dialogTime|ndTime|NCallsOut|NCallsIn|NCallsCorta|NAtend|NNoCalif|Available|PromDialog|twrapup'',''max'',1)
end
else begin
	update pivotReports set columns=''descripcion_time'',complementColumns=''date|login|user|sessionTime|loginMktTime|logoutMktTime|dialogTime|ndTime|NCallsOut|NCallsIn|NCallsCorta|NAtend|NNoCalif|Available|PromDialog|twrapup''
	where id=2100
end'
	EXEC(@sql)
...
EXEC ccspTmpTimesInterval @from = @from	,@to = @to	,@interval = 15 --Tabla Temporal para tener Intervalos de 15 Minutos
EXEC ccspTmpSessionGeneral @from = @from	,@to = @to				--Tabla para tener la sesiones de agentes
EXEC ccspTmpSessionTimeGroup @from = @from	,@to = @to				--Tabla para dividir la sesion en intervalos de 15 Minutos
EXEC ccspTimesccLogAgentesDia @from = @from	,@to = @to				--Tabla tener los movimientos de los agentes
EXEC ccspTimesOutboundData @from = @from	,@to = @to				--Tabla para los tiempos de las llamadas de salida
EXEC ccspTimesInboundData @from = @from	,@to = @to					--Tabla para los tiempos de las llamadas de entrada
exec ccspTmpTimesccLogtransfers @from = @from, @to = @to			--Tabla para los tiempos de las llamadas que son trasferidas
exec ccsptmpTimesHoldIn @from = @from, @to = @to					--Tabla para los tiempos cuando se pone en hold en llamadas de entrada
...
AS (
	...
	,callInStart as(
	select userId,MIN(dateIni) dateIni,callId,camType
	from tmpccLogAgentesDia 
	where callId>0 and  camType=0
	group by userId,callId,camType
	), callDataStartXfer as(
	select distinct A.userId,B.callId,B.camType,B.camId,B.dateEnd,B.currentStatus from callInStart A
	...
	,ISNULL(B.dateEnd,A.dateStartDetail) as dateXferAgtStart
	,DATEADD(ss, cal_txfer + cal_tring + cal_tdialog + cal_tnotas, ISNULL(B.dateEnd,A.dateStartDetail)) as dateEndDetail
	,cal_Ani, dni_id, User_id, statuscall_id, cal_que, cal_Xfer
	,cal_tMoh, cal_whoHung, calif_id, cal_puerto
	from inboundData A
	left join callDataStartXfer B on A.cal_id= B.callId and A.Inbound_id=B.camId
	)
...
from tmpccLogAgentesDia 
where callId>0 and camType=1
group by userId,callId,camType,camId
)
, callDataStartXfer as(
select distinct A.userId,B.callId,B.camType,B.camId,B.dateEnd,B.currentStatus from callOutStart A
inner join tmpccLogAgentesDia B on A.userId=B.userId and A.dateIni=B.dateIni 
where B.tStatus>0
)
,outData  as(
	...
	cal_Inicio AS dateStartDetail	
	, DATEADD(ss, cal_txfer + cal_tring + cal_tdialog + cal_tnotas, ISNULL(B.dateEnd,A.cal_Inicio)) AS dateEndDetail
	...
	, ISNULL(B.dateEnd,A.cal_Inicio) as dateXferAgtStart
	, DATEADD(ss, cal_txfer, ISNULL(B.dateEnd,A.cal_Inicio)) AS time_ring
	, DATEADD(ss, cal_txfer + cal_tring, ISNULL(B.dateEnd,A.cal_Inicio)) AS time_dialog
	, DATEADD(ss, cal_txfer + cal_tring + cal_tdialog, ISNULL(B.dateEnd,A.cal_Inicio)) AS time_notes
	, DATEADD(ss, cal_txfer + cal_tring + cal_tdialog + cal_tnotas, ISNULL(B.dateEnd,A.cal_Inicio)) AS time_end_call
...
Delete A
from tmpccLogAgentesDia A
inner join regDeleteRepLogout X  ON A.id = x.rowId AND A.userId = x.userId;
...
BEGIN
	...
	, isnull(co.tDialogOut, 0) + isnull(co.tNotesOut, 0) + isnull(ci.tDialogIn, 0) + isnull(ci.tNotesIn, 0) dialogTime
	...
	, ISNULL(a.sessionTime, 0) - ISNULL(r.notready, 0) AS Available
		,ISNULL(	
...
BEGIN
	...
	SELECT A.timegroup AS [date]
		...
		,isnull(A.tlog-(atgStatus.tunknown+atgStatus.tReady+atgStatus.tNotReady+atgStatus.tother+atgStatus.tprob+atgStatus.tchatting
		+txferin+txferout
		+tringin+tringout
		+tdialogin+tdialogout
		+tnotesIn+tnotesout
		),0) as tundefined
